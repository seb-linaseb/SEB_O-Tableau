...
Class Week2 {
  ...
  public function getWeekStartingDay() {
    $week = $this->week;
    $base_day = $this->base_day;              // Le 1er Janvier de l'année en cours
    $base_number = $this->base_number;        // Le numéro du jour du 1er Janvier de l'année en cours
    $week_number = $week;
  ...
  public function getWeekEndingDay() {
    ...
    // Calcul du décalage de semaine
    if($week_number == 1) {                 // Si on est sur la semaine 1
      $shift_number = $week - 1;            // Retourne un décalage de 1 semaine - 1 = 0 semaine
    } else {
      $shift_number = $week - 1;                // Retourne un décalage de x semaines
    }
